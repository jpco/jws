...
<p>
In the fall of 1990, one of us (Rakitzis) re-implemented the Plan 9 command interpreter, <i>rc</i>, for use as a UNIX shell.
Experience with that shell led us to wonder whether a more general approach to the design of shells was possible, and this paper describes the result of that experimentation.
We applied concepts from modern functional programming languages, such as Scheme and ML, to shells, which typically are more concerned with UNIX features than language design.
Our shell is both simple and highly programmable.
By exposing many of the internals and adopting constructs from functional programming languages, we have created a shell which supports new paradigms for programmers.

<p>
<b>Note:</b>
This web page is an HTML version of a paper which was presented at the Winter 1993 Usenix Conference in San Diego, California.
The paper corresponds to an out-of-date release of <i>es</i>; see the Errata section for changes which affect parts of the paper.
<a href="https://github.com/wryun/es-shell">Source code for the current version of <i>es</i></a> is available on GitHub.
...
<p>
While <i>rc</i> was an experiment in adding modern syntax to Bourne shell semantics, <i>es</i> is an exploration of new semantics combined with <i>rc</i>-influenced syntax: <i>es</i> has lexically scoped variables, first-class functions, and an exception mechanism, which are concepts borrowed from modern programming languages such as Scheme and ML. [<a class=local href=#ref4>4</a>, <a class=local href=#ref5>5</a>]

<p>
In <i>es</i>, almost all standard shell constructs (e.g., pipes and redirection) are translated into a uniform representation: function calls.
The primitive functions which implement those constructs can be manipulated the same way as all other functions: invoked, replaced, or passed as arguments to other functions.
The ability to replace primitive functions in <i>es</i> is key to its extensibility; for example, a user can override the definition of pipes to cause remote execution, or the path-searching machinery to implement a path look-up cache.
...
<p>
For simple commands, <i>es</i> resembles other shells.
For example, newline usually acts as a command terminator.
These are familiar commands which all work in <i>es</i>:
...
For this reason, the reader is referred to the paper on <i>rc</i> for a discussion of quoting rules, redirection, and so on.
(The examples shown here, however, will try to aim for a lowest common denominator of shell syntax, so that an understanding of <i>rc</i> is not a prerequisite for understanding this paper.)

<h2><a id=functions>Functions</a></h2>

<p>
<i>Es</i> can be programmed through the use of shell functions.
Here is a simple function to print the date in <i>yy-mm-dd</i> format:
...
<i>Es</i> allows parameters to be specified to functions by placing them between the function name and the open-brace.
This function takes a command <code>cmd</code> and arguments <code>args</code> and applies the command to each argument in turn:
...
<p>
Note that <code>apply</code> was called with more than two arguments; <i>es</i> assigns arguments to parameters one-to-one, and any leftovers are assigned to the last parameter.
For example:
...
<p>
If there are fewer arguments than parameters, <i>es</i> leaves the leftover parameters null:
...
<p>
So far we have only seen simple strings passed as arguments.
However, <i>es</i> functions can also take program fragments (enclosed in braces) as arguments.
For example, the <code>apply</code> function defined above can be used with program fragments typed directly on the command line:
...
<p>
is just a way of inlining a function on the command-line.
This is called a <em>lambda</em>. [<a class=local href=#footnote2 name=footnote2-use>Footnote 2</a>]
It takes the form
...
<p>
directly at the shell, and this runs the inlined function directly on the argument <code>/tmp</code>.

<p>
There is one more thing to notice: the inline function that was supplied to <code>apply</code> had a parameter named <code>i</code>, and the <code>apply</code> function itself used a reference to a variable called <code>i</code>.
Note that the two uses did not conflict: that is because <i>es</i> function parameters are
<i>lexically scoped</i>, much as variables are in C and Scheme.

<h2><a id=variables>Variables</a></h2>

<p>
The similarity between shell functions and lambdas is not accidental.
In fact, function definitions are rewritten as assignments of lambdas to shell variables.
Thus these two <i>es</i> commands are entirely equivalent:
...
<p>
In order not to conflict with regular variables, function variables have the prefix <code>fn-</code> prepended to their names.
This mechanism is also used at execution time; when a name like <code>apply</code> is seen by <i>es</i>, it first looks in its symbol table for a variable by the name <code>fn-apply</code>.
Of course, it is always possible to execute the contents of any variable by dereferencing it explicitly with a dollar sign:
...
<p>
The previous examples also show that variables can be set to contain program fragments as well as simple strings.
In fact, the two can be intermixed:
...
This makes variables into versatile tools.
For example, a variable could be used as a function dispatch table.

<h2><a id=binding>Binding</a></h2>

<p>
In the section on functions, we mentioned that function parameters are lexically scoped.
It is also possible to use lexically-scoped variables directly.
For example, in order to avoid interfering with a global instance of <code>i</code>, the following scoping syntax can be used:
...
<p>
One use of lexical binding is in redefining functions.
A new definition can store the previous definition in a lexically scoped variable, so that it is only available to the new function.
This feature can be used to define a function for tracing calls to other functions:
...
<p>
The <code>trace</code> function redefines all the functions which are named on its command line with a function that prints the function name and arguments and then calls the previous definition, which is captured in the lexically bound variable <code>old</code>.
Consider a recursive function <code>echo-nl</code> which prints its arguments, one per line:
...
<p>
matches <var>subject</var> against <var>pattern</var> and returns true if the subject is the same as the pattern.
(In fact, the matching is a bit more sophisticated, for the pattern may include wildcards.)

<p>
Shells like the Bourne shell and <i>rc</i> support a form of local assignment known as <em>dynamic binding</em>.
...
<p>
In addition to the prefix (<code>fn-</code>) for function execution described earlier, <i>es</i> uses another prefix to search for <i>settor variables</i>.
A settor variable <code>set-</code><i>foo</i> is a variable which gets evaluated every time the variable <i>foo</i> changes value.
A good example of settor variable use is the <code>watch</code> function:
...
<p>
<code>Watch</code> establishes a settor function for each of its parameters; this settor prints the old and new values of the variable to be set, like this:
...
<p>
The first function, <code>cons</code>, returns a function which takes as its argument another function to run on the parameters <code>a</code> and <code>d</code>.
<code>car</code> and <code>cdr</code> each invoke the kind of function returned by <code>cons</code>, supplying as the argument a function which returns the first or second parameter, respectively.
For example:
...
<p>
By providing a routine which catches <code>error</code> exceptions, a programmer can intercept internal shell errors before the message gets printed.

<p>
Exceptions are also used to implement the <code>break</code> and <code>return</code> control flow constructs, and to provide a way for user code to interact with UNIX signals.
While six error types are known to the interpreter and have special meanings, any set of arguments can be passed to <code>throw</code>.

<p>
Exceptions are trapped with the built-in <code>catch</code>, which typically takes the form
...
<code>%create</code> is the built-in function which opens <code>/tmp/foo</code> on file-descriptor 1 and runs<code>ls</code>.

<p>
The value of this rewriting is that the <code>%create</code> function (and that of just about any other shell service) can be <i>spoofed</i>, that is, overridden by the user: when a new <code>%create</code> function is defined, the default action of redirection is overridden.

<p>
Furthermore, <code>%create</code> is not really the built-in file redirection service.
It is a hook to the <i>primitive</i> <code>$&amp;create</code>, which itself cannot be overridden.
That means that it is always possible to access the underlying shell service, even when its hook has been reassigned.
...
<p>
In fact, most redefinitions do not refer to the <code>$&amp;</code>-forms explicitly, but capture references to them with lexical scoping.
Thus, the above redefinition would usually appear as
...
<p>
Overriding traditional shell built-ins is another common example of spoofing.
For example, a <code>cd</code> operation which also places the current directory in the title-bar of the window (via the hypothetical command <code>title</code>) can be written as:
...
<p>
Spoofing can also be used for tasks which other shells cannot do; one example is timing each element of a pipeline by spoofing <code>%pipe</code>, along the lines of the pipeline profiler suggested by Jon Bentley[<a class=local href=#ref7>7</a>]; see Figure 1.
...
<p>
One other piece of <i>es</i> which can be replaced is the interpreter loop.
In fact, the default interpreter is written in <i>es</i> itself; see Figure 3.
...
<p>
A few details from this example need further explanation.
The exception <code>retry</code> is intercepted by <code>catch</code> when an exception handler is running, and causes the body of the <code>catch</code> routine to be re-run.
<code>%parse</code> prints its first argument to standard error, reads a command (potentially more than one line long) from the current source of command input, and throws the <code>eof</code> exception when the input source is exhausted.
The hook <code>%prompt</code> is provided for the user to redefine, and by default does nothing.

<p>
Other spoofing functions which either have been suggested or are in active use include: a version of <code>cd</code> which asks the user whether to create a directory if it does not already exist; versions of redirection and program execution which try spelling correction if files are not found; a <code>%pipe</code> to run pipeline elements on (different) remote machines to obtain parallel execution; automatic loading of shell functions; and replacing the function which is used for tilde expansion to support alternate definitions of home directories.
Moreover, for debugging purposes, one can use <code>trace</code> on hook functions.

<h2><a id=implementation>Implementation</a></h2>

<p>
<i>Es</i> is implemented in about 8000 lines of C.
Although we estimate that about 1000 lines are devoted to portability issues between different versions of UNIX, there are also a number of work-arounds that <i>es</i> must use in order to blend with UNIX.
The <code>path</code> variable is a good example.

<p>
The <i>es</i> convention for path searching involves looking through the list elements of a variable called <code>path</code>.
This has the advantage that all the usual list operations can be applied equally to <code>path</code> as any other variable.
However, UNIX programs expect the path to be a colon-separated list stored in <code>PATH</code>.
Hence <i>es</i> must maintain a copy of each variable, with a change in one reflected as a change in the other.
...
<p>
which bind the shell services such as short-circuit-and, backgrounding, etc., to the <code>%</code>-prefixed hook variables.
...
<p>
A note on implementation: these functions temporarily assign their opposite-case settor cousin to null before making the assignment to the opposite-case variable.
This avoids infinite recursion between the two settor functions.
...
<p>
which lexically binds <code>b</code> to the variable <code>a</code> for the scope of this function definition.
Therefore, the external representation of this function must make this information explicit.
It is encoded as:
...
<p>
Unlike most traditional shells, which have feature sets dictated by the UNIX system call interface, <i>es</i> contains features which do not interact well with UNIX itself.
For example, rich return values make sense from shell functions (which are run inside the shell itself) but cannot be returned from shell scripts or other external programs, because the <i>exit</i>/<i>wait</i> interface only supports passing small integers.
This has forced us to build some things into the shell which otherwise could be external.
...
We consider this unfortunate, but there seemed no reasonable way to tie exception propagation to any existing UNIX mechanism.
In particular, the signal machinery is unsuited to the task.
...
This does not turn out to be a significant problem, but it does not seem intuitive to a programmer with a background in functional languages.

<p>
One restriction on <i>es</i> that arose because it had to work in a traditional UNIX environment is that lists are not hierarchical; that is, lists may not contain lists as elements.
In order to be able to pass lists to external programs with the same semantics as passing them to shell functions, we had to restrict lists to the same structure as <i>exec</i>-style argument vectors.
Therefore all lists are flattened, as in <i>rc</i> and <i>csh</i>.

<h2><a id=garbage-collection>Garbage Collection</a></h2>

<p>
Since <i>es</i> incorporates a true lambda calculus, it includes the ability to create true recursive structures, that is, objects which include pointers to themselves, either directly or indirectly.
While this feature can be useful for programmers, it has the unfortunate consequence of making memory management in <i>es</i> more complex than that found in other shells.
...
<p>
During normal execution of the shell, memory is acquired by incrementing a pointer through a pre-allocated block.
When this block is exhausted, all live pointers from outside of garbage collector memory, the <i>rootset</i>, are examined, and any structure that they point to is copied to a new block.
When the rootset has been scanned, all the freshly copied data is scanned similarly, and the process is repeated until all reachable data has been copied to the new block.
At this point, the memory request which triggered the collection should be able to succeed.
...
<p>
Garbage collectors have developed a reputation for being hard to debug.
The collection routines themselves typically are not the source of the difficulty.
Even more sophisticated algorithms than the one found in <i>es</i> are usually only a few hundred lines of code.
Rather, the most common form of GC bug is failing to identify all elements of the rootset, since this is a rather open-ended problem which has implications for almost every routine.
To find this form of bug, we used a modified version of the garbage collector which has two key features: (1) a collection is initiated at every allocation when the collector is not disabled, and (2) after a collection finishes, access to all the memory from the old region is disabled. [<a class=local href=#footnote3 name=footnote3-use>Footnote 3</a>]
...
<p>
There are two performance implications of the garbage collector; the first is that, occasionally, while the shell is running, all action must stop while the collector is invoked.
This takes roughly 4% of the running time of the shell.
...
It is hard to quantify the performance consequences of this restriction.
...
<p>
There are several places in <i>es</i> where one would expect to be able to redefine the built-in behavior and no such hook exists.
The most notable of these is the wildcard expansion, which behaves identically to that in traditional shells.
We hope to expose some of the remaining pieces of <i>es</i> in future versions.

<p>
One of the least satisfying pieces of <i>es</i> is its parser.
We have talked of the distinction between the core language and the full language; in fact, the translation of <i>syntactic sugar</i> (i.e., the convenient UNIX shell syntax presented to the user) to core language features is done in the same <i>yacc</i>-generated parser as the recognition of the core language.
Unfortunately, this ties the full language in to the core very tightly, and offers little room for a user to extend the syntax of the shell.

<p>
We can imagine a system where the parser only recognizes the core language, and a set of exposed transformation rules would map the extended syntax which makes <i>es</i> feel like a shell, down to the core language.
The <i>extend-syntax</i> [<a class=local href=#ref9>9</a>] system for Scheme provides a good example of how to design such a mechanism, but it, like most other macro systems designed for Lisp-like languages, does not mesh well with the free-form syntax that has evolved for UNIX shells.

<p>
The current implementation of <i>es</i> has the undesirable property that all function calls cause the C stack to nest.
In particular, tail calls consume stack space, something they could be optimized not to do.
Therefore, properly tail recursive functions, such as <code>echo-nl</code> above, which a Scheme or ML programmer would expect to be equivalent to looping, have hidden costs.
...
<p>
There are two central ideas behind <i>es</i>.
The first is that a system can be made more programmable by exposing its internals to manipulation by the user.
By allowing spoofing of heretofore unmodifiable shell features, <i>es</i> gives its users great flexibility in tailoring their programming environment, in ways that earlier shells would have supported only with modification of shell source itself.

<p>
Second, <i>es</i> was designed to support a model of programming where code fragments could be treated as just one more form of data.
This feature is often approximated in other shells by passing commands around as strings, but this approach requires resorting to baroque quoting rules, especially if the nesting of commands is several layers deep.
In <i>es</i>, once a construct is surrounded by braces, it can be stored or passed to a program with no fear of mangling.
...
The keyword <code>@</code> introduces the lambda.
Since <code>@</code> is not a special character in <i>es</i> it must be surrounded by white space.
<code>@</code> is a poor substitute for the Greek letter lambda, but it was one of the few characters left on a standard keyboard which did not already have a special meaning.
...
<p>
This section covers changes to <i>es</i> since original publication of the paper.
If you are aware of any undocumented differences, please contact the authors.
...
<code>error</code> exceptions now have an additional piece of information.
The second word (the one after <code>error</code>) is now the name of the routine which caused the error.
Thus, in the new version of <code>in</code> below, the <code>throw</code> command has an extra <code>in</code> in it.

<p>
<a id=erratum4 class=local href=#erratum4-use>4.</a>

This example uses an obsolete version of the <code>fork</code> builtin.
The <code>in</code> function should now be
...
<p>
When <i>es</i> is built, an executable called <code>esdump</code> is created, which is a trimmed down version of the shell.
That program is run, with the initialization file <code>initial.es</code> as its input.
...
<p>
Paul Haahr is a computer scientist at Adobe Systems Incorporated where he works on font rendering technology.
His interests include programming languages, window systems, and computer architecture.
Paul received an A.B. in computer science from Princeton University in 1990.
He can be reached by electronic mail at <i>haahr@adobe.com</i> or by surface mail at Adobe Systems Incorporated, 1585 Charleston Road, Mountain View, CA 94039.  [<a class=local href=#erratum1>Errata note 1</a>]
