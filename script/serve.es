...
# This is a slapdash dev http server written in the extensible shell.
...
(method reqpath version) = <={%split ' ' <=%read}
...
fn reply code type {
	...
	let (
		...
		echo >[1=2] $method $reqpath '->' HTTP/1.1 $code $(code-$code)
		echo HTTP/1.1 $code $(code-$code)
...
# This needs to be consistent with app.yaml/init.go for deployment to actually,
# like, work.
#

if {access -f static/$reqpath} {
	serve static/$reqpath
} {access -f static/$reqpath/index.html} {
	serve static/$reqpath/index.html
} {
	reply 404 text/plain
	echo Path ''''$reqpath'''' not found.
}
